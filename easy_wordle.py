...
def filter_word_list(words, clues):
    """This function takes in a list of words and clue list by using the brute force method
    The output will a new word list containing the right word in the list """
    newlist = []
    
    # check the list of color
    for word in words:
        flag = True
        for guess, clue in clues:
            guess = guess.upper()
            word = word.upper()
            if (check_word(word, guess) != clue): # checking with the color in the tuples
                flag = False
                break
        if flag == True:
...
if __name__ == "__main__":
    ...
    for i in range(6): # 6 guesses count
        ...
        if (guess == secret):
            ...
            clues.append((guess, check_word(secret, guess))) # appending tupple to clues
            possiblewords = filter_word_list(words, clues)
            print(len(possiblewords), "words possible:")
            random.shuffle(possiblewords)
            if len(possiblewords) < 6: # 5 words only
